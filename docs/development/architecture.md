...
- **Character manager**: a `CharacterPreset` data type that pairs a
  ...
  code.
- **LLM client** (`src/app/llm/`): a tiny OpenAI-compatible HTTP client
...
- **i18n module**: a tiny `t(key, …)` lookup against the locale table
  loaded from `src/assets/i18n/<locale>.lua`. The active locale lives in
...
src/
  ...
  conf.lua
tests/                  -- pure-Lua unit tests against src/core and src/app
platform/
  macos/
  linux/
  ios/
...
> A move is decided by `src/app/ai/` from `src/core/` state and nothing
> else. A chat line is generated by `src/app/llm/` from game state and
...
- **No platform-conditional code in `src/core` or `src/app`.** Anything
  platform-specific lives in `src/ui` or `platform/`.
