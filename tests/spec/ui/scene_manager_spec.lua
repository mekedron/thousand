...
describe("ui.scene_manager", function()
    ...
    it("round-trips a session reference and clears it", function()
        assert.is_nil(m:session())
        assert.is_false(m:is_game_active())
        local fake_session = { tag = "session" }
        m:set_session(fake_session)
        assert.are.equal(fake_session, m:session())
    ...
    it("preserves the session reference across scene transitions", function()
        m:switch_to("a")
        local fake_session = { tag = "session" }
        m:set_session(fake_session)
        m:switch_to("b")
