...
describe("app.bot.stubs", function()
    ...
    describe("choose_cut_deck", function()
        it("invokes the canonical cut", function()
            local action = stubs.choose_cut_deck(fake_view({}), 2)
            assert.are.equal("cut_deck", action.kind)
            assert_kind_in_registry("choose_cut_deck", action.kind)
        end)
    end)
